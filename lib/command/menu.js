...
cmd.on('menu-cmd', ['menu','help','command','commands'], [], async (msg, {
    ...
    let tit = list_now == 0 ? `${upper}${list} ${list}${line.repeat(5)}${list} ${list_now+1} ${list}${line.repeat(5)}${list}`:`${body}${list} ${list}${line.repeat(5)}${list} ${list_now+1} ${list}${line.repeat(5)}${list}`
    sections.push({
    "title": tit,
    "rows": [{
