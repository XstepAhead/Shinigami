...
},{wait: true, query:'Isi Link SOUNDCLOUD \nContoh : /soundcloud https://soundcloud.com/linkin_park/in-the-end',param:functions.needed('link')});

cmd.on('mediafire',['mediafire'],['downloader'],async(msg,{query,client}) =>   {
let res = (await functions.axios.get(botinfo.linkApi.zacros+"/downloader/mediafireDl?link="+query)).data
await client.sendText(msg.from, functions.parseResult(res,{title:"Mediafire Downloader"}))
return client.sendMessage(msg.from, res.link, "documentMessage", {mimetype:"application/json", filename:res.title}).catch(e => client.reply(msg,`Terjadi kesalahan, media yang kamu kirimkan error/melebihi size`))
},{wait: true, query:'Isi Link Mediafire',param:functions.needed('link')});

cmd.on('sfile',['sfile','solidfiles'],['downloader'],async(msg,{query,client}) =>   {
let res = (await functions.axios.get(botinfo.linkApi.zacros+"/downloader/sfiledl?link="+query)).data
await client.sendText(msg.from, functions.parseResult(res,{title:"Solidfiles Downloader"}))
return client.sendMessage(msg.from, res.url, "documentMessage", {mimetype:"application/json", filename:res.title}).catch(e => client.reply(msg,`Terjadi kesalahan, media yang kamu kirimkan error/melebihi size`))
