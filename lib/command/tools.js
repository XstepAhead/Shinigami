...
cmd.on("ssweb",["ss","ssweb","sshp"],["tools"],async(req,res)=>{
  if (!res.query) return client.sendText(req.from, botinfo.response.notUrl)
  if (res.command.match["ss","ssweb"]) {
return client.sendImage(req.from, botinfo.linkApi.zacros+"/tools/ssweb?link="+res.query)
  }
  if (res.command.match("sshp")) {
    return client.sendImage(req.from, botinfo.linkApi.zacros+"/tools/sshp?link="+res.query)
  }
